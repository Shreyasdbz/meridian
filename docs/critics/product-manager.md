...
The document describes a system of extraordinary ambition: six custom components, a dual-LLM safety architecture, a self-improving plugin system, three types of persistent memory with vector search, container sandboxing, encrypted credential vaults, OWASP-compliant security, WCAG-accessible UI, Prometheus metrics, five LLM provider integrations, and adaptive model selection. All of this targeting a Raspberry Pi.
...
The document describes approximately 18-24 months of full-time engineering work for a team of 3-4 experienced developers. For a solo developer, this is a multi-year project. Every section adds "just one more" critical feature:
...
Section 5.5.6 mentions a "setup wizard" in one sentence. This is the most critical UX moment in the entire product and it gets seven words. What does the wizard ask? How many steps? Does the user need to paste an API key immediately, or can they explore first? What if they do not have an API key yet?
...
How quickly does the user see Meridian do something useful? If the first task requires Scout to plan, Sentinel to validate, and Gear to execute, the user is waiting 10-30 seconds with no feedback. Is there a progress indicator? What if Sentinel blocks the first task and the user has to approve something they do not understand yet?
...
- Is willing to configure two separate LLM providers with separate API keys.
- Understands what "dual-LLM trust boundary" means.
- Has the technical knowledge to review Gear permission manifests.
- Is cost-sensitive enough to want a Raspberry Pi but willing to pay for two LLM API calls per task.

This person does not exist in meaningful numbers.

**The Raspberry Pi crowd** wants something that works like Home Assistant: install it, click through a UI, and automate their lights. They do not want to review JSON permission manifests or understand information barriers.
...
For a user's first autonomous task, the request flows through:
...
12. Journal reflects (another LLM call, 2-10 seconds).
13. Bridge displays result.

Best case for a simple task: 6-30 seconds of wall time with three separate LLM API calls. Worst case (user approval needed): indefinite.

Compare this to Open Interpreter: "Type a command, get a result in 2-5 seconds."

### Recommendation

For v0.1, collapse the pipeline:

- Fast path should be the default for most tasks.
- For full-path tasks, skip Sentinel (use rule-based policies) and skip Journal reflection.
- The user should see their first result in under 5 seconds.
...
A new user must understand: Axis, Scout, Sentinel, Journal, Bridge, Gear, fast path, full path, journal-skip, Sentinel Memory, Gear Synthesizer, execution plans, validation results, risk levels, permission manifests, provenance tags, information barriers, and the loose schema principle.

That is 18 concepts before they type their first message.
...
A Raspberry Pi user (the stated primary target) is cost-sensitive by definition. They chose a $75 computer. Now consider the per-task costs:

| Step | Model | Estimated Cost |
|---|---|---|
| Scout planning | Claude Sonnet | $0.003-0.015 per task |
| Sentinel validation | GPT-4o | $0.002-0.010 per task |
| Journal reflection | Claude Sonnet | $0.003-0.015 per task |
| Embeddings | API-based | $0.001 per task |

That is $0.009-0.041 per task. At 50 tasks per day (moderate usage), that is $0.45-2.05 per day, or $13.50-61.50 per month.

The document acknowledges this and offers mitigations (fast path, adaptive models, Sentinel Memory, local models). But the mitigations add complexity without eliminating the cost. A user running Claude Sonnet for Scout + GPT-4o for Sentinel + reflection is spending $30-60/month on API calls for a "free, self-hosted" assistant.

For comparison, ChatGPT Plus is $20/month and requires zero setup.
...
The architecture document provides exactly zero built-in Gear for any of these. The built-in Gear set is: file-manager, web-search, web-fetch, shell, scheduler, notification. That is a file system browser and a web scraper.

The gap between "manage my calendar" and "here is a file manager and a shell command executor" is enormous. The implicit assumption is that Journal's Gear Synthesizer will create integrations on the fly. This is an extraordinarily ambitious bet. Writing reliable Gmail API integration code is not a simple LLM task. OAuth flows, token refresh, API pagination, error handling, rate limits -- LLMs generate plausible-looking code for these but it rarely works correctly on the first try.
...
- The doc says private IP ranges are blocked for Gear. Smart home devices are on private IPs. How is this resolved?
...
- Every step requires user approval by default (shell execution, file writes outside workspace).
- A simple scaffolding task would require 10+ approval clicks. Is that acceptable?
...
Approximately 70% of what is described in the architecture document should not be built before the first user touches the product. Build the core loop, get it in someone's hands, and then decide what matters based on real usage.
...
Users are being asked to give an AI agent access to their files, email, and shell on their personal device. The architecture addresses this with security layers, but trust is built through track record, not architecture documents. A new, unaudited open-source project asking for shell access is a hard sell, no matter how good the sandbox documentation is.
...
The most successful open-source projects have compelling demos. "Watch this agent check my email and summarize it" is compelling. "Watch this agent go through a six-step pipeline with independent safety validation" is not. The architecture optimizes for safety at the expense of demo-ability.
...
Section 3 ("Lessons from OpenClaw") is 70 lines of detailed competitive teardown, including specific CVE numbers, malware rates ("6.9% malware rate"), and a comparison table that paints the competitor as comprehensively inferior.

This section has several problems:

**It reveals insecurity, not confidence.** A confident architecture document describes what it does and why. It does not spend 10% of its length explaining why a competitor is bad. Apple did not launch the iPhone with a section about why BlackBerry's keyboard was a security risk.

**It locks the architecture into reactive design.** Several Meridian design decisions are framed as "responses" to OpenClaw failures rather than first-principles decisions. This creates an architecture shaped by someone else's mistakes rather than by user needs. "We encrypt secrets because OpenClaw did not" is weaker than "We encrypt secrets because our users store credentials for their most sensitive accounts."
...
| 13 | Default Sentinel to off and Journal to off to minimize per-task API cost. | High |
| 14 | Drop Raspberry Pi as the primary target. "Also works on a Pi" is fine. | Medium |
